/**
 * @file main.c
 * @brief Main program loop for the FrankenText text generator.
 *
 * This program will:
 *   - prepare tokens and successor tables,
 *   - building a list of valid sentence start tokens (uppercase letter),
 *   - and generating random sentences until the stop condition '?','!' or '.'
 * is met.
 *
 * Part of the assignment for DTU course 62712: Basic C Programming (Week 08).
 * Author: Mikkel Olsen
 * Date: November 2025
 */

#include <ctype.h>
#include <stdbool.h>
#include <stddef.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>

#define MAX_WORD_COUNT 15'000
#define MAX_SUCCESSOR_COUNT MAX_WORD_COUNT / 2

char book[] = {
#embed "pg84.txt" /// Stores the content of the file as an array of chars.
    , '\0'};      /// Makes `book` a string.

/// Array of tokens registered so far.
/// No duplicates are allowed.
char *tokens[MAX_WORD_COUNT];
/// `tokens`'s current size
size_t tokens_size = 0;

/// Array of successor tokens
/// One token can have many successor tokens. `succs[x]` corresponds to
/// `token[x]`'s successors.
/// We store directly tokens instead of token_ids, because we will directly
/// print them. If we wanted to delete the book, then it would make more sense
/// to store `token_id`s
char *succs[MAX_WORD_COUNT][MAX_SUCCESSOR_COUNT];
/// `succs`'s current size
size_t succs_sizes[MAX_WORD_COUNT];

////////////////////////////////////
/// Array and counter, used by the new added function
/// build_start_token_list
///////////////////////////////////////
static size_t start_token_ids[MAX_WORD_COUNT]; // Index list of tokens which
                                               // start with uppercase
static size_t start_token_count = 0;           // Number of stored indices

/// Overwrites non-printable characters in `book` with a space.
/// Non-printable characters may lead to duplicates like
/// `"\xefthe" and "the"` even both print `the`.
void replace_non_printable_chars_with_space() {
  for (size_t i = 0; i < strlen(book); ++i) {
    if (!isprint(book[i])) {
      book[i] = ' ';
    }
  }
}

/// Returns the id (index) of the token, creating it if necessary.
///
/// Returns token id if token exists in \c tokens, otherwise creates a new entry
/// in \c tokens and returns its token id.
///
/// \param token token to look up (or insert)
/// \return Index of `token` in \c tokens array
size_t token_id(char *token) {
  size_t id;
  for (id = 0; id < tokens_size; ++id) {
    if (strcmp(tokens[id], token) == 0) {
      return id;
    }
  }
  tokens[id] = token;
  ++tokens_size;
  return id;
}

/// Appends the token \c succ to the successors list of \c token.
void append_to_succs(char *token, char *succ) {
  auto next_empty_index_ptr = &succs_sizes[token_id(token)];

  if (*next_empty_index_ptr >= MAX_SUCCESSOR_COUNT) {
    printf("Successor array full.");
    exit(EXIT_FAILURE);
  }

  succs[token_id(token)][(*next_empty_index_ptr)++] = succ;
}

/// Creates tokens on \c book and fills \c tokens and \c succs using
/// the functions \c token_id and \c append_to_succs.
void tokenize_and_fill_succs(char *delimiters, char *str) {
  // YOUR CODE HERE
  char *prev_tok = NULL;
  char *tok = strtok(str, delimiters); // For the first word

  // While loop : As long there is some thing in tok, RUN
  while (tok != NULL) {
    (void)token_id(tok); // Saving the word if new
    if (prev_tok != NULL) {
      append_to_succs(prev_tok, tok); // Add current 'tok' to the 'previoustok':
    }
    prev_tok = tok; // Setting the current tok to be the prev_tok
    tok = strtok(NULL, delimiters); // starting the on the New tok
  }
}

/// Returns last character of a string
char last_char(char *str) {
  // YOUR CODE HERE
  size_t len = strlen(str);
  return (len == 0) ? '\0' : str[len - 1];
}

/// Returns whether the token ends with `!`, `?` or `.`.
bool token_ends_a_sentence(char *token) {
  // YOUR CODE HERE
  char c = last_char(token);
  return (c == '.') || (c == '!') || (c == '?');
}

/**
 * @brief  New added function: Builds the list of token indices that can start a
 * sentence. The list is used by random_token_id_that_starts_a_sentence().
 *
 * A token is added to `start_token_ids[]` if:
 *  - its first character is uppercase,
 *  - it does not end a sentence ('.', '!' or '?'),
 *  - and it has at least one successor token (so the sentence can continue).
 *
 */
static void build_start_token_list(void) {
  start_token_count = 0;
  for (size_t i = 0; i < tokens_size; ++i) {
    if (isupper((unsigned char)tokens[i][0]) &&
        !token_ends_a_sentence(tokens[i]) && succs_sizes[i] > 0) {
      start_token_ids[start_token_count++] = i;
    }
  }
}

/// Returns a random `token_id` that corresponds to a `token` that starts with a
/// capital letter.
/// Uses \c tokens and \c tokens_size.
size_t random_token_id_that_starts_a_sentence(void) {
  if (start_token_count == 0)
    return 0;
  return start_token_ids[rand() % start_token_count];
}

/// Generates a random sentence using \c tokens, \c succs, and \c succs_sizes.
/// The sentence array will be filled up to \c sentence_size-1 characters
/// using random tokens until:
/// - a token is found where \c token_ends_a_sentence
/// - or more tokens cannot be concatenated to the \c sentence anymore.
/// Returns the filled sentence array.
///
/// @param sentence array what will be used for the sentence.
//
//                  Will be overwritten. Does not have to be initialized.
/// @param sentence_size
/// @return input sentence pointer
char *generate_sentence(char *sentence, size_t sentence_size) {
  size_t current_token_id = random_token_id_that_starts_a_sentence();
  auto token = tokens[current_token_id];

  sentence[0] = '\0';
  strcat(sentence, token);
  if (token_ends_a_sentence(token))
    return sentence;

  // Calculated sentence length for the next iteration.
  // Used to stop the loop if the length exceeds sentence size
  size_t sentence_len_next;
  // Concatenates random successors to the sentence as long as
  // `sentence` can hold them.
  do {
    // Selecting the successors
    size_t succ_count = succs_sizes[current_token_id];
    if (succ_count == 0)
      break;
    char *next = succs[current_token_id][rand() % succ_count];

    // // Determining the total length of the next sentence with a space in
    // between
    size_t cur_len = strlen(sentence);
    size_t next_len = strlen(next);
    sentence_len_next = cur_len + 1 + next_len;

    if (sentence_len_next >=
        sentence_size - 1) // checking if there is room for '\0', else break out
                           // of the while
      break;

    // Adding a space and the next word with memcpy
    sentence[cur_len] = ' ';
    memcpy(sentence + cur_len + 1, next, next_len);
    sentence[sentence_len_next] = '\0';

    current_token_id = token_id(next); // Word used for next round

    // checking if the next word is the end of a sentence, else break out of the
    // while.
    if (token_ends_a_sentence(tokens[current_token_id]))
      break;

  } while (sentence_len_next < sentence_size - 1);

  return sentence;
}

int main() {
  // Init tokens: Using memset to ensure that the the arrays a cleared.
  memset(tokens, 0, sizeof tokens);
  memset(succs, 0, sizeof succs);
  memset(succs_sizes, 0, sizeof succs_sizes);

  replace_non_printable_chars_with_space();

  char *delimiters = " \n\r";
  tokenize_and_fill_succs(delimiters, book);

  build_start_token_list(); // Building the list of words with uppercase
                            // letters.

  char sentence[1000];
  srand(time(NULL)); // Be random each time we run the program

  // Generate sentences until we find a question sentence.
  int test = 0;

  do {
    generate_sentence(sentence, sizeof sentence);
    test++;
    printf("\n%d", test);
  } while (last_char(sentence) != '?');
  puts(sentence);
  puts("");

  // Initialize `sentence` and then generate sentences until we find a
  // sentence ending with an exclamation mark.
  do {
    generate_sentence(sentence, sizeof sentence);
  } while (last_char(sentence) != '!');
  puts(sentence);
}